#!/usr/bin/env python3
"""Convert a FIX Orchestra XML instance into LinkML-validatable YAML/JSON.

The generated LinkML schema (``src/fix_orchestra/schema/fix_orchestra.yaml``)
expects data in JSON-shaped form, but the upstream test instances are XML
(``<fixr:repository>``, ``<fixi:interfaces>``). This script bridges them so
``linkml-validate`` can ingest a FIX-Trading-Community sample directly.

Transformations applied:

* Strip namespace prefixes from tags and attributes
  (``fixr:``, ``fixi:``, ``dc:``, ``dct:``, ``xml:``, ``xsi:``).
...
* Collapse single-child mixed content ``<dc:title>FIX</dc:title>`` into a
  bare string ``"FIX"`` so it matches the schema's ``range: string``.
* Bucket unrecognised attributes into the class's ``extra_attributes`` slot
  if present (mirrors XSD ``<xs:anyAttribute>``).

Usage::

    python3 scripts/fix_xml_to_linkml.py \
        --schema src/fix_orchestra/schema/fix_orchestra.yaml \
        --target-class Repository \
        --in tests/data/third_party/fix-orchestra/mit_2016.xml \
        --out /tmp/mit_2016.yaml

If ``--target-class`` is omitted, it's inferred from the XML root tag
(``repository`` -> ``Repository``, ``interfaces`` -> ``Interfaces``).
...
def infer_target_class(xml_path: Path) -> str:
    root_tag = local_name(ET.parse(xml_path).getroot().tag)
    return _ROOT_TO_CLASS.get(root_tag, pascal(root_tag))


def main(argv: list[str] | None = None) -> int:
    here = Path(__file__).resolve().parent
    project_dir = here.parent
    default_schema = (project_dir / 'src' / 'fix_orchestra' / 'schema'
                      / 'fix_orchestra.yaml')
