...
# Generation date: 2026-05-21T08:04:28
...
class CategoryType(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if self._is_empty(self.section):
            self.MissingRequiredField("section")
        if not isinstance(self.section, Name):
...
slots.categoryType__section = Slot(uri=FIXR.section, name="categoryType__section", curie=FIXR.curie('section'),
                   model_uri=FIX_ORCHESTRA.categoryType__section, domain=None, range=Union[str, Name])
...
slots.UniqueInline_field_ref = Slot(uri=FIXR.fieldRef, name="UniqueInline_field_ref", curie=FIXR.curie('fieldRef'),
                   model_uri=FIX_ORCHESTRA.UniqueInline_field_ref, domain=UniqueInline, range=Optional[Union[Union[dict, FieldRefType], list[Union[dict, FieldRefType]]]])
...
slots.GroupType_impl_min_occurs = Slot(uri=FIXR.implMinOccurs, name="GroupType_impl_min_occurs", curie=FIXR.curie('implMinOccurs'),
                   model_uri=FIX_ORCHESTRA.GroupType_impl_min_occurs, domain=GroupType, range=Optional[int])
...
slots.GroupType_which = Slot(uri=FIXR.which, name="GroupType_which", curie=FIXR.curie('which'),
                   model_uri=FIX_ORCHESTRA.GroupType_which, domain=GroupType, range=Optional[Union[str, "MemberType"]])
...
slots.MessageType_when = Slot(uri=FIXR.when, name="MessageType_when", curie=FIXR.curie('when'),
                   model_uri=FIX_ORCHESTRA.MessageType_when, domain=MessageType, range=Optional[Union[str, ExpressionType]])
